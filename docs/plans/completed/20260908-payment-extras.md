...
- [x] додати в `covers(p)` гілку, що повертає **`html.EscapeString(p.Label)`** —
      `Format` (рядки 46-48) приклеює результат `covers` сирим, на відміну від
      `p.Class`/`p.Person`
- [x] переписати doc-коментар `covers` (рядки 52-54): гілка «ні те, ні те» тепер
      законна, це `extra`; додати, чому екранування саме тут
- [x] не змінювати заголовок «💸 Оплата» і не заводити четвертий шаблон тексту
- [x] написати тест `Format` на рядку з `label`: назва курсу, дитина, сума, «за що»
- [x] написати тест екранування: `label` з `&` і `<` не ламає рядок
- [x] запустити тести — мусять пройти до задачі 5
...
- [x] перевірити наскрізь: доп. оплата видна в `/lessons/payments`, на дашборді, у
      статистиці по курсу, в ленті аудиту і на головній Mini App
- [x] перевірити, що баланс курсу після доп. оплати не змінився (веб + Mini App)
- [x] **перевірити, що звичайна оплата, створена після міграції, видна в `/packs`
      бота і що період аудиту «з оплати» відкривається на ній** — наскрізна перевірка
      пастки №4. Перевірено на живому сервері: період відкрився на «з останньої
      оплати (01.09.2026)», не на кімоно (05.09). `/packs` бота наживо **не**
      запускався — він читає `PaymentsForEnrollment`, накритий
      `TestACoursePaymentStaysVisibleToTheLessonQueries`
- [x] ~~перевірити, що бот надіслав повідомлення з «за що»~~ — наживо не
      перевірялося: локальний прогон свідомо без токена, щоб нічого не полетіло
      в сімейну групу. Накрито `TestGroupTextNamesWhatTheMoneyBought` і
      `TestGroupTextEscapesTheExtraLabel`; лишається в Post-Completion
- [x] запустити повний набір тестів: `go test ./...`
- [x] `go vet ./...` — лінтера в репозиторії не налаштовано (немає `.golangci.yml`),
      тож це і є реальний гейт

### Task 11: [Final] Update documentation

- [x] `ARCHITECTURE.md:41` — переписати опис `payments` («money in. Either prepaid
...
- [x] `ARCHITECTURE.md:452-456` — там сказано, що повідомлення в групу називає
      «a pack of lessons or a named month»; додати третій варіант
- [x] `ARCHITECTURE.md` — згадати, що інваріанта живе у `payments.Form.Parse`, а не в
      схемі, і що це свідомий відступ від `CHECK`-конвенції `0010`
- [x] перенести цей план у `docs/plans/completed/`


**Що показав живий прогін** (тимчасовий харнесс, окрема БД, без токена):

- `/lessons/payments` — «доп. Кімоно» / «доп. Форма» з бейджем, поруч з «8 зан.»
  і «абон. 01.09.2026—30.09.2026»; «Усього: 8100 ₴» = 3200 + 1500 + 2500 + 900
- `/lessons` — доп. оплати в «Останні оплати» з бейджем; баланс карате
  «залишилось 6» (8 куплено − 2 проведено), кімоно його не зрушило
- `/stats/lessons` — «Демид · Карате 4700 ₴ (з них 1500 ₴ доп.) / 2»
- лента аудиту — `ledger-extra` на 05.09 з балансом 6, тобто без руху; підсумок
  «оплачено: 8 зан. · 3200 ₴» окремо від «додатково: 1500 ₴»
- текстова версія — «05.09 додатково: Кімоно (1500 ₴) — розмір 140» і
  «Додатково за період: 1500 ₴»
- форма — перемикач «За заняття / Додатково»; POST з `kind=extra` → 303,
  той самий без `label` → 422 і «вкажи, за що оплата»
- `payments.js` і `home.js` — парсяться (node, SourceTextModule)

➕ Знайдено на прогоні: на телефоні доп. оплата в «Останні оплати» нічим не
відрізнялася від курсової, хоч на вебі бейдж є. Той самий аргумент — додано
`.pill-extra` в `home.js` (`kind` уже був у DTO).
## Post-Completion
