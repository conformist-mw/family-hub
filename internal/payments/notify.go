...
// covers says what the money bought — a pack of lessons, a named month, or,
// for an extra, whatever it was for. Empty when the row says none of the
// three, which the forms do not allow but an old imported row might.
//
// The extra's label is escaped here rather than by the caller. Format escapes
// p.Class and p.Person at their own call sites and appends this result raw,
// which was safe while everything returned from here was generated by the app
// — a numeral or a month name. A label is typed by a person, so an "&" in it
// would make Telegram reject the whole message; the write has already
// succeeded by then, so the group would simply never hear about the payment.
