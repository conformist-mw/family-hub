...
func (a *App) handleAppointmentCreate(w http.ResponseWriter, r *http.Request) {
	appt, date, hhmm, formErr := parseAppointmentForm(r)
	if formErr != "" {
		a.renderAppointmentFormError(w, appt, date, hhmm, false, formErr)
		return
	}
...
func (a *App) handleAppointmentUpdate(w http.ResponseWriter, r *http.Request) {
	...
	appt.ID = id
	if formErr != "" {
		a.renderAppointmentFormError(w, appt, date, hhmm, true, formErr)
		return
	}
...
func parseAppointmentForm(r *http.Request) (model.Appointment, string, string, string) {
	...
	if appt.Title == "" {
		return appt, date, hhmm, "вкажи назву"
	}
	if _, err := time.ParseInLocation(model.LocalDatetime, date+"T"+hhmm, time.Local); err != nil {
		return appt, date, hhmm, "вкажи коректну дату й час"
	}
	appt.StartsAt = date + "T" + hhmm
	if endTime != "" {
		if _, err := time.ParseInLocation(model.LocalDatetime, date+"T"+endTime, time.Local); err != nil {
			return appt, date, hhmm, "вкажи коректний час завершення"
		}
		if endTime <= hhmm { // same-day only: an appointment crossing midnight isn't a thing here
			return appt, date, hhmm, "час завершення має бути пізніше початку"
		}
		appt.EndsAt = date + "T" + endTime
	}
	if !isValidApptStatus(appt.Status) {
		return appt, date, hhmm, "вибери статус"
	}
	return appt, date, hhmm, ""
}

func (a *App) renderAppointmentFormError(w http.ResponseWriter, appt model.Appointment, date, hhmm string, isEdit bool, msg string) {
